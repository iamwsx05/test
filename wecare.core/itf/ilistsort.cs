...
namespace weCare.Core.Itf
{
    ...
    public class IListSort<T>
    {
        ...
        public IListSort(IList<T> list, string[] propertyName)
        {
            ...
            if (_sortBy == null)
            {
                _sortBy = new bool[_propertyName.Length];
            }
        ...
        private int CompareOne(T x, T y, PropertyInfo property, bool sortBy)
        {
            ...
            else if (property.PropertyType.FullName.Contains("System.Decimal"))
            {
                ...
                if (property.GetValue(x, null) != null)
                {
                    dec1 = weCare.Core.Utils.Function.Dec(property.GetValue(x, null));
                }
                if (property.GetValue(y, null) != null)
                {
                    dec2 = weCare.Core.Utils.Function.Dec(property.GetValue(y, null));
                }
                if (sortBy)
                {
